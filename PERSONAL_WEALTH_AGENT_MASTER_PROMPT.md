...
- ห้ามเปลี่ยนชื่อหรือลบ Account ที่ Transaction, Goal หรือ Investment รุ่นใหม่ยังอ้างถึง

#### 7.8 Account-linked Investments

- Investment ใหม่ต้องเลือก `account_from` และระบุ `funded_amount > 0`
- ลด `Accounts.balance` แต่ไม่ถูกนับเป็น Expense หรือ Cash Flow
- แก้รายการต้องย้อนผลเดิมก่อนใช้ผลใหม่
- ลบรายการต้องคืน `funded_amount` เข้าบัญชีต้นทางก่อนลบ
- Investment เก่าที่ `account_from` และ `funded_amount` ว่างเป็น Legacy และห้ามปรับ Opening Balance ย้อนหลัง
- `funded_amount` คือเงินต้นที่หักจาก Account ส่วน `current_value` คือมูลค่าตลาดปัจจุบัน
